...
def main():
    ...
    try:
        ...
        def should_include_finding(finding, tool):
            """Filter out low/info severity findings to focus on actionable security issues"""
            # Always include gitleaks findings (secrets are always critical)
            if tool == 'gitleaks':
                return True
        ...
        for tool, findings in results.items():
            if findings:
                filtered_findings = [f for f in findings if should_include_finding(f, tool)]
        ...
        logger.info("📝 Generating PR comment...")
...
def _write_pr_findings(results, output_dir, total_findings, ai_suggestions, time_saved, cost_savings, repo_path):
    ...
    for tool, findings in pr_filtered_results.items():
        ...
        for f in findings:
            ...
            if tool == 'gitleaks':
                msg = f.get("Description", "Secret detected")
    ...
    pr_file_path = output_dir / f"pr-findings.txt"
...
"""
    ...
    (output_dir / f"executive-summary.md").write_text(summary)
...
Generated by ImagineX AppSec AI Scanner"""
    ...
    # Write Slack summary
    (output_dir / f"slack-executive-summary.txt").write_text(summary)
